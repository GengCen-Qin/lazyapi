...
func FastAPI(g *gocui.Gui, v *gocui.View, method int) error {
	FormInfo.Active = true
	FormInfo.Fields = []string{"path", "params"}
	FormInfo.CurrentField = 0
	FormInfo.IsFastApi = true
	FormInfo.FastMethod = method

	if err := createFormContainer(g); err != nil {
		return err
	}

	if err := setupFormTitle(g, method); err != nil {
		return err
	}
...
func ShowNewAPIForm(g *gocui.Gui, v *gocui.View) error {
	...
	if err := setupFormTitle(g, entity.Method_Unkown); err != nil {
		return err
	}
...
func setupFormTitle(g *gocui.Gui, method int) error {
	...
	} else if FormInfo.IsFastApi {
		form_view.Title = "快速请求模式(" + entity.MethodTitle[method] + ")"
	} else {
...
// getFieldPosition 获取字段位置
func getFastApiFieldPosition(field string, maxX, maxY int) (x1, y1, x2, y2 int) {
	switch field {
	case "path":
		return maxX/6 + 1, maxY/6 + 2, maxX*5/6 - 1, maxY/6 + 4
	case "params":
		return maxX/6 + 1, maxY/6 + 5, maxX*5/6 - 1, maxY*5/6 - 1
	default:
		return 0, 0, 0, 0
	}
}
...
func createFormField(g *gocui.Gui, field string) error {
	...
	if FormInfo.IsFastApi {
		x1, y1, x2, y2 = getFastApiFieldPosition(field, maxX, maxY)
	} else {
...
func CloseForm(g *gocui.Gui, v *gocui.View) error {
	...
	// 重新设置焦点到左侧视图
	if _, err := SetCurrentViewOnTop(g, common.ViewArr[common.ViewActiveIndex]); err != nil {
		return err
	}
	if FormInfo.IsFastApi {
		FormInfo.Reset()
...
func validateAPIForm(g *gocui.Gui, name, path, method, params string) error {
	...
	if !FormInfo.IsFastApi && name == "" {
		emptyFields = append(emptyFields, "name")
	}
	if !FormInfo.IsFastApi && method == "" {
	...
	if !FormInfo.IsFastApi && (method != "GET" && method != "POST") {
